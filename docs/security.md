...
PyMuPDF extraction has a page cap, rejects password-protected PDFs with safe codes, strips NUL bytes, and never logs document text. Pages without native text are selectively passed to local Tesseract with configured language, DPI, and page-budget limits; every stored page records whether text came from `NATIVE`, `OCR`, or `NONE`. Retrieval requires the authenticated `company_id` and `user_id`, filters company, active principal, assignment, published training, requested document, and latest `READY` version before cosine ranking. PDF content is wrapped as untrusted evidence and the system prompt explicitly rejects instructions embedded in sources. Generated citations are validated against retrieved chunks before the API exposes document version, title, page, excerpt, and score.

An acknowledgement is accepted only from an assigned employee for the latest published document version. The evidence row snapshots identity, fixed attestation, version, filename, checksum, and timestamp and has no update/delete endpoint. Repeated requests for the same user/version are idempotent, stale-version requests fail, and a training with acknowledgement evidence cannot be deleted.
